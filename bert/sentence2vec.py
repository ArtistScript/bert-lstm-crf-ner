...
flags.DEFINE_integer("feature_pooling_layer", -2, 
    "Sentence feature is pooled from the specific layer, default=-2")
...
def main(_):
  ...
  with codecs.getwriter("utf-8")(tf.gfile.Open(FLAGS.output_file,"w")) as writer:
    for result in estimator.predict(input_fn, yield_single_examples=True):
      ...
      writer.write("\t".join([str(unique_id), "".join(text), ",".join([str(x) for x in encodes])]) + "\n")
